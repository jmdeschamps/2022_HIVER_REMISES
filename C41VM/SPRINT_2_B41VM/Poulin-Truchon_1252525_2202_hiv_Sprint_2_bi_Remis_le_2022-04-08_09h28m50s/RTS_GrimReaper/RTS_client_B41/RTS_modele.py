...
import math
import time
# If not working -> cmd prompt ---     pip install playsound=1.2.2
...
class Daim():
    def __init__(self, parent, id, x, y, notyperegion=-1, idregion=None):
        ...
        self.x = x
        self.y = y
        self.valeur = 40   ##cuanto recurso te da
    ...
    def trouver_cible(self): ##Alex pour que le kid reaper attire les ames
        ...
        for i in clefj:
            ...
            for j in ListKD:
                ...
                dist = Helper.calcDistance(self.x, self.y, j.x, j.y) ##pour calculer la distance relativement au kid reaper
...
class Joueur():
    ...
    def transferer_soul(self, param):  # Yoan
        ...
        ames = self.batiments[USINEBALLISTE][id].nbr_soul  # Yoan
        ...
        self.batiments[USINEBALLISTE][id].nbr_soul = 0  # On remet la banque d'âmes de l'usine à 0 après le transfert

        if self.ressources["soul"] >= 220:  #SS
    ...
    def creer_point_origine(self, x, y):
        ...
        print(idmaison)
        print(idballiste)
        self.objet_porte = self.batiments[MAISON][idmaison] = Maison(self, idmaison, self.couleur, x, y, MAISON)  #alex pour que le porte sois un choix de batiment mere
        self.objet_usine = self.batiments[USINEBALLISTE][idballiste] = Usineballiste(self, idballiste, self.couleur, x + 60, y + 60, USINEBALLISTE)  # création de la ferme et on la met dans une variable YOAN ALEX
        ...
        self.persos["ballista"][id] = Joueur.classespersos["ballista"](self, id, self.objet_usine, self.couleur, x + 60, y + 60, "ballista")
        self.ressources["ballista"] += 1
        #self.ressources[MAISON] += 1 # SS (valeur ajoutée directement dans self.ressources de joueur)
...
class Partie():
    ...
    def __init__(self, parent, mondict):
        ...
        self.on_off = 0  ##Alex? pour le choix de retour de l'grim\ pas tres clair comme variable
    ...
    def determiner_jour_nuit(self):
        ...
        if (t - self.debut) % DAY == 0 and self.daylight is True:
            ...
            print("C'est la nuit")
            playsound('musique/LauraPalmer.mp3', False)  # A TRAVAILLER YOAN
            print('LauraPalmerTheme')
            ...
            print("C'est le jour")
            playsound('musique/DayLight.mp3', False)  # A TRAVAILLER YOAN
            print('DayLightTheme')
