...
####### INTERFACES GRAPHIQUES
    ...
    def creer_quetes(self):
        ...
        #fichierquetes.close()
        nbNourriture = self.modele.joueurs[self.parent.monnom].ressources["nourriture"]
        nbBatiments = self.modele.joueurs[self.parent.monnom].q2
        nbDronesAttaque = 0

        if self.modele.joueurs[self.parent.monnom].ressources["aureus"] > self.nbtempBitcoin:
            self.nbBitcoin += self.modele.joueurs[self.parent.monnom].ressources["aureus"] - self.nbtempBitcoin
            self.nbtempBitcoin = self.modele.joueurs[self.parent.monnom].ressources["aureus"]
        elif self.modele.joueurs[self.parent.monnom].ressources["aureus"] < self.nbtempBitcoin:
            self.nbtempBitcoin = self.modele.joueurs[self.parent.monnom].ressources["aureus"]
        ...
        #nbBitcoin = self.modele.joueurs[self.parent.monnom].ressources["aureus"]
        ...
        with open('quetes.txt', mode = 'r') as csv_file:
            ...
            for row in csv_reader:
                ...
                if line_count == 0 and nbNourriture >= 300:
                    self.q1done = True
    ...
    def afficher_jeu(self):
        ...
        for j in self.modele.joueurs.keys():
            ...
            if j==self.parent.monnom:
                ...
                self.infohud["Gem"][0].set(self.modele.joueurs[j].ressources["gem"])
            ...
            for p in self.modele.joueurs[j].persos.keys():
                for k in self.modele.joueurs[j].persos[p].keys():
                    ...
                    if p=="ballista":
                        for b in self.modele.joueurs[j].persos[p][k].fleche:
                            self.canevas.create_image(b.x,b.y,image=self.images[b.image],
                                                      tags=("mobile",j,b.id,"",type(b).__name__,""))
        ...
        for j in self.modele.troupeEnnemis["troupeEnnemis"].keys():
            ...
            if i.etat == "mort":
                ...
                self.canevas.create_image(i.x, i.y, image=self.images[i.img],
                                          tags=("mobile", "npc", i.id, i.montype))
                #self.parent.chercher_ouvrier()

            # dessiner javelot de troupe ennemis
            for t in i.javelots:
    ...
    def creer_entite(self,evt):
        ...
        if self.parent.monnom in mestags:
            if "batiment" in mestags:
                ...
                self.parent.actionsrequises.append(action)
            # if "potion" in mestags :
            #     if "vie1" in mestags :
            #         pos=(self.canevas.canvasx(evt.x),self.canevas.canvasy(evt.y))
            #         action=[self.parent.monnom,"acheter",["marchand",mestags[4],mestags[2],pos]]
            #     self.parent.actionsrequises.append(action)
