...
####### INTERFACES GRAPHIQUES
    ...
    def creer_cadre_lobby(self):
        ...
        self.canevaslobby=Canvas(self.cadrelobby,width=640,height=480,bg="lightblue2")
    ...
    def creer_aire_de_jeu(self):
        ...
        self.canevas.create_oval(1700, 2200, 2300, 2800, disabledoutline="#990000", state="disabled")
...
##### FONCTIONS DU SPLASH #########################################################################
    ...
    def afficher_depart(self):
        ...
        for i in self.modele.npcs:
            if i.montype == "marchand":
                self.canevas.create_image(i.x, i.x, image=self.images["marchand"], tags=("statique","npc", i.id, i.montype))
    ...
    def afficher_jeu(self):
        ...
        i=self.modele.boss
        if i.sprite:
            self.canevas.create_image(i.x,i.y,image=self.gifs["boss"][i.spriteno],
                                        tags=("mobile","",i.id,"boss",i.montype))
...
##### ACTIONS DU JOUEUR #######################################################################
    ...
    def chasser_ressource(self,evt):
        ...
        if tag[1]=="" and self.action.persochoisi and tag[4]=="daim":
            ...
            self.action.chasser_ressource(tag)
        elif tag[1]=="npc" and self.action.persochoisi and tag[3] == "boss":
            self.action.chasser_ressource(tag)
        else:
            print(tag[1])
            print(tag[3])
    ...
    def event(self):
        ...
        if nombre == 0:
            ...
            messagebox.showinfo(title="E V E N T !", message="Un largage est arrivé, chercher le pour une récompense !")
        if nombre == 1:
            for i in self.modele.npcs:
    ...
    #def verifier_sorti_zone_boss(self):
    #    persos = self.modele.joueurs[self.parent.monnom].persos
    #    for k in persos.keys():
    #        for i in persos[k]:
    #            for j in self.modele.npcs:
    #                if j.montype == "boss":
    #                    if (j.x + j.champvision >= persos[k][i].x >= j.x - j.champvision) and (j.y + j.champvision >= persos[k][i].y >= j.y - j.champvision):
    #                        self.dansZoneBoss = False
    #                    else:
    #                        self.dansZoneBoss = True
    #                    if self.dansZoneBoss:
    #                        j.v = j.vmax
    ...
    def batir_artefact(self,evt):
        ...
        if ok:
            ...
            self.action.btnactif.config(bg="SystemButtonFace")
            messagebox.showinfo(title="Vous êtes pauvre !", message="Nombre de ressources insuffisant")
