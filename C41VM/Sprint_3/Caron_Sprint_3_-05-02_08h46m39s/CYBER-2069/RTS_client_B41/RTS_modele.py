...
class Boss():
    def __init__(self, parent, id, montype):
        ...
        self.actioncourante = "chercherouvrier"
        self.etats_et_actions={"chercherouvrier": self.chercher_ouvrier,
    ...
    def jouer_prochain_coup(self):
        ...
        if self.actioncourante == "chercherouvrier":
            self.chercher_ouvrier()
    ...
    def chercher_ouvrier(self):
        for i in self.parent.joueurs:
            for j in self.parent.joueurs[i].persos["ouvrier"]:
                j = self.parent.joueurs[i].persos["ouvrier"][j]
                if j != self.cible:
                    distance = Helper.calcDistance(self.x, self.y, j.x, j.y)
                    # si ouvrier dans son champ de vision
                    if distance <= self.champvision:
                        # cibler ouvrier
                        self.cibler_ouvrier(j)

    def cibler_ouvrier(self, ouvrier):
        self.cible = ouvrier
    ...
    def attaquer(self):
        ...
        if self.cible.etat == "vivant":
            ...
            self.actioncourante = "chercherouvrier"
            self.parent.eliminer_ressource("ouvrier", self.cible)
            self.cible = None
            self.position_visee = None
...
class Perso():
    def __init__(self, parent, id, batiment, couleur, x, y, montype):
        ...
        self.attaque = 0 + self.parent.statistique["attaque"]
        self.fondVie = None
        self.rectVie = None
        self.montype = montype
        self.cible = None
...
class Chevalier(Perso):
    ...
    def bouger(self):
        if self.position_visee:
            ...
            #for k in persos.keys():
            #    for i in persos[k]:
            #        if persos[k][i].montype == "chevalier":
            #            dir = "DB"
            #            if 0 <= angquad <= 89:
            #                dir = "DB"
            #            elif -90 <= angquad <= -1:
...
class Ingenieur(Perso):
    def __init__(self, parent, id, maison, couleur, x, y, montype):
        ...
        self.cible=None
        self.angle=None
        self.champchasse = 200
        self.distennemi = 200
        self.javelots = []
        self.cibleennemi = None
        self.nomimg="ingenieur"
        self.couleur = couleur[0]
        self.typeressource = None
        self.cibletemp = None
        self.etats_et_actions = {"bouger": self.bouger,
    ...
    def bouger(self):
        if self.position_visee:
            ...
            #for k in persos.keys():
            #    for i in persos[k]:
            #        if persos[k][i].montype == "ingenieur":
            #            dir = "DB"
            #            if 0 <= angquad <= 89:
            #                dir = "DB"
            #            elif -90 <= angquad <= -1:
...
class Ballista(Perso):
    def __init__(self, parent, id, maison, couleur, x, y, montype):
        ...
        #self.ang = Helper.calcAngle(self.x, self.y, self.x, self.y)
        #self.angquad = math.degrees(self.ang)
    ...
    def chercher_nouvelle_ressource(self, typ, idreg):
        if typ != "baie" and  typ != "daim" and typ != "troupeEnnemis" and typ != "boss":
            ...
            if typ == "troupeEnnemis":
                ...
                return None
            if typ == "boss":
                # on ne cherche rien après avoir tué un boss, en attente d'une prochaine action
                return None
            else :
...
class Joueur():
    ...
    def acheter_potion(self, param):
        ...
        if nomPotion == "vie1":
            ...
            # selon le perso (parcourir la liste) ajouter valeur à leur stat
            #for k in self.persos.keys():
            #    for i in self.persos[k]:
            #        self.persos[k][i].vmax += self.potions["vie1"]
            #        print(self.persos[k][i].vmax)
...
class Partie():
    ...
    def __init__(self, parent, mondict):
        ...
        self.npcs = [] #comprend marchand et boss
    ...
    def jouer_prochain_coup(self, cadrecourant):
        ...
        for i in self.troupeEnnemis["troupeEnnemis"].keys():
            ...
            #self.troupeEnnemis["troupeEnnemis"][i].deplacer()
            #self.troupeEnnemis["troupeEnnemis"][i].chercher_ouvrier()
