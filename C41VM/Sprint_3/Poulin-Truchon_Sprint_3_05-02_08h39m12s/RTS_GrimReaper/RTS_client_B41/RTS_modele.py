    ...
    def deplacer(self):
        if self.etat == "mort":
            ...
            self.angle = Helper.calcAngle(self.x + random.randrange(100, 200),
                                          ...
                                          self.cible.y + random.randrange(100, 200)) # Alex pour que les daim gravite autour du kid
...
class RogueReaper(Perso):
    ...
    def retour_batiment_mere(self):
        ...
        if reponse == "rendu":
            if self.cible:
                if self.typeressource == "grave" or self.typeressource == "daim" or self.typeressource == "brume":
    ...
    def cibler_proie(self):
        ...
        reponse = self.bouger()
        if reponse == "rendu":
            if self.typeressource == "daim" or self.typeressource == "brume":
                self.actioncourante = "ramasserressource"
...
class Grim(Perso):
    ...
    def temps_cibler_ressource(self):
        if not self.parent.parent.daylight:
            self.cibler_ressource()
    ...
    def chercher_nouvelle_ressource(self, typ, idreg):
        print("Je cherche nouvelle ressource")
        if typ != "grave" and typ != "daim" and typ != "grave_empty":
            reg = self.parent.parent.regions[typ]
            if idreg in reg:
                regspec = self.parent.parent.regions[typ][idreg]
                n = len(regspec.dicocases)
...
class Joueur():
    ...
    def transferer_soul(self, param):  # Yoan
        ...
        ames = self.batiments[SOULFARM][id].nbr_soul  # Yoan
