...
class Vue():
    ...
    def afficher_bio(self, bio):
        ...
        if bio.montype == 'grave':
            self.canevas.create_image(bio.x, bio.y - 50, image=self.images[bio.img2],
                                      tags=("statique", "", bio.id, "biotope", bio.montype, ""))  # YOAN pour les âmes au dessus des tombes
        ...
        if self.parent.monnom in mestags:
            if "batiment" in mestags:
                ...
                if SOULFARM in mestags:
                    if self.parent.modele.joueur[mestags[1]].ressources["funeral"] == 0:
                        pos = (self.canevas.canvasx(evt.x), self.canevas.canvasy(evt.y))
                        action = [self.parent.monnom, "creerperso", ["grim", mestags[4], mestags[2], pos]]
                    else:
