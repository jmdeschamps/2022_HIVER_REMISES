...
#Noms : Matis Gaetjens, Gabriel Tremblay, Antonin Lenoir, Jane Caron
...
class Controleur():
    def __init__(self):
        ...
        self.event_valide = False
        self.spawn_troupe = False
    ...
    def initialiser_partie(self, mondict):
        ...
        random.seed()  # random FIXE pour test ou ...
    ...
    def boucler_sur_jeu(self):
        ...
        self.vue.creer_barrevie()
        self.vue.creer_quetes()
        t = int(time.time())
        # Un event va apparaitre à chaque 10 minutes
        if (t - self.modele.retourner_debut()) % 600 == 0 and self.event_valide: # on fait un boolean parce que sinon il passe plusieurs fois dans le if
            self.event_valide = False
            self.vue.event()
        elif (t - self.modele.retourner_debut()) % 600 == 1:
            self.event_valide = True
        self.vue.verifier_proche_coffre()
        self.vue.verifier_proche_pnj()
        if (t - self.modele.retourner_debut()) % 30 == 0 and self.spawn_troupe:
            self.spawn_troupe = False
            self.modele.creer_troupeEnnemis()
        elif (t - self.modele.retourner_debut()) % 30 == 2:
            self.spawn_troupe = True
        #self.vue.verifier_sorti_zone_boss()
    ...
    #def chercher_ouvrier(self):
        #self.modele.chercher_ouvrier()

    def trouver_valeursMarchand(self):
        vals = self.modele.trouver_valeursMarchand()
        return vals

    def acheter_potion(self, nomPotion):
        self.modele.acheter_potion(nomPotion)
