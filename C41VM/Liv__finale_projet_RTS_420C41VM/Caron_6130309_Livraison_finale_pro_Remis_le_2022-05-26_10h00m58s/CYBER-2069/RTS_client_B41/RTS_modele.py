...
class Potion():
    def __init__(self, parent, id, montype):
        self.parent = parent
        self.id = id
        self.montype = montype
        self.image = None
        #type : attaque, vie, defense, vitesse
...
class Marchand():
    def __init__(self, parent, id, montype):
        self.parent = parent
        self.id = id
        self.monnom = "marchand"
        self.montype = "marchand"
        self.x = self.parent.aireX / 2
        self.y = self.parent.aireY / 2

        self.sprite = "marchand"
        self.spriteno = 0
        self.spritelen = len(self.parent.parent.vue.gifs["marchand"])
...
class MarchandEvent():
    def __init__(self, parent):
        self.parent = parent
        self.id = get_prochain_id()
        self.nomimg = "marchandevent"
        self.montype = "marchandevent"
        self.etat = ""
        self.img = ""
        self.x = (self.parent.aireX / 2) - 100
        self.y = (self.parent.aireY / 2) - 100
        self.cout = 0
        self.ressource = ""
...
class Boss():
    def __init__(self, parent, id, montype):
        ...
        self.sprite = "boss"
        self.spriteno = 0
        self.spritelen = len(self.parent.parent.vue.gifs["boss"])
        self.dir = ""
        # self.img = self.nomimg + self.dir
        self.vitesse = random.randrange(3) + 3
        self.actioncourante = "chercherunite"
        self.etats_et_actions={"chercherunite": self.chercher_unite,
                               "attaquer": self.attaquer,
                               }

        n = random.randrange(50)
        if n == 6:
            self.spritelen = 6 #len(self.parent.parent.vue.gifs["boss"])
            self.sprite = "boss"
    ...
    def jouer_prochain_coup(self):
        ...
        if self.actioncourante == "chercherunite":
            self.chercher_unite()
        elif self.actioncourante == "attaquer":
            self.attaquer()
    ...
    def chercher_unite(self):
        for j in self.parent.joueurs:
            for k in self.parent.joueurs[j].persos.keys():
                for i in self.parent.joueurs[j].persos[k]:
                    unite = self.parent.joueurs[j].persos[k][i]
                    if unite != self.cible:
                        distance = Helper.calcDistance(self.x, self.y, unite.x, unite.y)
                        # si ouvrier dans son champ de vision
                        if distance <= self.champvision:
                            # cibler
                            self.cibler_unite(unite)

    def cibler_unite(self, ouvrier):
        self.cible = ouvrier
        self.position_visee = [self.cible.x, self.cible.y]
        self.actioncourante = "attaquer"

    def attaquer(self):
        if self.cible.etat == "vivant":
            self.valider_projectile()
            self.actioncourante = "attaquer"
        elif self.cible.etat != "vivant":
            self.actioncourante = "chercherunite"
            self.parent.eliminer_ressource(self.cible.montype, self.cible)
            self.cible = None
            self.position_visee = None

    def valider_projectile(self):
        self.lancer_projectile(self.cible)
        for i in self.javelots:
                i.bouger()

    def lancer_projectile(self, proie):
        if not self.javelots:
            id = get_prochain_id()
            self.javelots.append(Javelot(self, self.a, proie))
...
class TroupeEnnemis():
    def __init__(self, parent, id, x, y, notyperegion=-1, idregion=None):
        ...
        self.id = id
        self.etat = "vivant"
        self.nomimg = "troupeEnnemis"
        self.montype = "troupeEnnemis"
        ...
        self.rectBouc = None
        self.valeur = 40
        self.vmax = 50  # si on veut éventuellement changer leur stat de base (augmenter la difficulté)
        self.v = self.vmax
        self.bouclier = 0
        self.b = self.bouclier
        self.attaque = 20
        self.a = self.attaque # pour possible affichage et modification future de l'attaque
        self.position_visee = None
        self.angle = None
        self.dir = "GB"
        self.img = self.nomimg + self.dir
        ...
        self.etats_et_actions={"deplacer": self.deplacer,
                               "bouger": self.bouger,
                               "attaquer": self.attaquer,
                               }
    ...
    def jouer_prochain_coup(self):
        if self.actioncourante == "deplacer":
            self.deplacer()
        elif self.actioncourante == "bouger":
            self.bouger()
        elif self.actioncourante == "attaquer":
            self.attaquer()
    ...
    def deplacer(self):
        if self.position_visee:
            ...
            if self.etat == "vivant":
                self.trouver_cible_terrain()
        self.chercher_unite()

    def trouver_cible_terrain(self):
        n = 1
        while n:
    ...
    def bouger(self):
        if self.position_visee:
            # le if sert Ã  savoir si on doit repositionner notre visee pour un objet
            ang = Helper.calcAngle(self.x, self.y, self.cible.x, self.cible.y)
    ...
    def attaquer(self):
        reponse = self.bouger()
        if reponse <= self.distennemi and self.cible.etat == "vivant":
            self.valider_javelot()
            self.actioncourante = "attaquer"
        elif self.cible.etat != "vivant":
            self.actioncourante = "deplacer"
            self.parent.eliminer_ressource(self.cible.montype, self.cible)
            ...
            self.position_visee = None
            for i in self.javelots:
                self.javelots.remove(i)
        elif reponse > self.distennemi:
            self.actioncourante = "deplacer"
            self.cible = None
            self.position_visee = None
            for i in self.javelots:
...
class Javelot():
    def __init__(self, parent, attaquant_attaque, proie):
        self.parent = parent
        self.id = id
    ...
    def bouger(self):
        ...
        if self.proie.etat != "vivant":
            self.parent.javelots.remove(self)
        elif dist <= self.taille:
            # enlever bouclier d'abord
            if self.proie.b > 0:
                self.proie.b -= self.attaquant_attaque
                print(self.proie.b)
                self.parent.javelots.remove(self)
                return self.proie.b
            # si bouclier = 0, enlever de la vie
            elif self.proie.v > 0:
                self.proie.v -= self.attaquant_attaque
                self.parent.javelots.remove(self)
                #if self.proie.v <= 0:
                    #self.proie.mourir
                    #self.parent. eliminer_ressource(self.cible.montype, self.cible)
            ...
            if dist < self.vitesse:
                ...
                self.parent.javelots.remove(self)
            return self.proie.v
    #def infliger_degats(self, vieCible):
...
class Perso():
    ...
    def bouger(self):
        if self.position_visee:
            ...
            x1, y1 = Helper.getAngledPoint(ang, self.vitesse, self.x, self.y)
            angquad = math.degrees(ang)
            persos = self.parent.persos
                ...
                for i in persos[k]:
                    if persos[k][i].montype == "ouvrier":
                        ...
                        if -90 <= angquad <= 90:
                            ...
                            dir = "G"
                        persos[k][i].image = persos[k][i].couleur + "_" + persos[k][i].montype + dir
            ...
            dist = Helper.calcDistance(self.x, self.y, x, y)
            if self.cible == None or self.cible.etat == None:
                self.x, self.y = x1, y1
            elif dist >= self.distennemi or self.cible.etat != "vivant":
                self.x, self.y = x1, y1
            if dist <= self.vitesse:
...
class Soldat(Perso):
    def __init__(self, parent, id, maison, couleur, x, y, montype):
        ...
        self.vmax = 60 + self.parent.statistique["vie"] # vie initiale
        self.v = self.vmax  # vie courante
        self.bouclier = 0 + self.parent.statistique["defense"]
        self.b = self.bouclier # bouclier courant
        self.attaque = 10 + self.parent.statistique["attaque"]
        self.a = self.attaque # attaque courante
        self.idregion = None
        self.force = 20
        self.dir = "D"
        self.image = couleur[0] + "_" + montype + self.dir
    ...
    def cibler_proie(self):
        ...
        if self.cible.etat == "vivant":
            reponse = self.bouger()
            if reponse <= self.champchasse and self.cible.etat == "vivant":
    ...
    def jouer_prochain_coup(self):
        if self.actioncourante and self.actioncourante != "ciblerressource":
            reponse = self.etats_et_actions[self.actioncourante]()
    ...
    def bouger(self):
        if self.position_visee:
            if self.cible == None:
                ...
                angquad = math.degrees(ang)
                persos = self.parent.persos
                # changer l'image lors du changement de direction
                for k in persos.keys():
                    for i in persos[k]:
                        if persos[k][i].montype == "soldat":
                            dir = "D"
                            if 0 <= angquad <= 89:
                                dir = "D"
                            elif -90 <= angquad <= -1:
                ...
                dist = Helper.calcDistance(self.x, self.y, self.cible.x, self.cible.y)
                if self.cible is None or self.cible.etat is None:
                    self.x, self.y = x1, y1
                elif dist >= self.distennemi:
                    self.x, self.y = x1, y1
                return dist
...
class Druide(Perso):
    def __init__(self, parent, id, maison, couleur, x, y, montype):
        Perso.__init__(self, parent, id, maison, couleur, x, y, montype)
        self.vmax = 55 + self.parent.statistique["vie"] # vie initiale
        self.v = self.vmax  # vie courante
        self.bouclier = 0 + self.parent.statistique["defense"]
        self.b = self.bouclier # bouclier courant
        self.attaque = 9 + self.parent.statistique["attaque"]
        self.a = self.attaque # attaque courante
        self.idregion = None
        self.dir = "D"
        self.image = couleur[0] + "_" + montype + self.dir
...
class Ballista(Perso):
    def __init__(self, parent, id, maison, couleur, x, y, montype):
        Perso.__init__(self, parent, id, maison, couleur, x, y, montype)
        self.vmax = 80 + self.parent.statistique["vie"] # vie initiale
        self.v = self.vmax  # vie courante
        self.bouclier = 0 + self.parent.statistique["defense"]
        self.b = self.bouclier # bouclier courant
        self.attaque = 18 + self.parent.statistique["attaque"]
        self.a = self.attaque # attaque courante
        self.idregion = None
        self.dir = "DH"
        self.image = couleur[0] + "_" + montype + self.dir
    ...
    def bouger(self):
        if self.position_visee:
            if self.cible == None:
                ...
                for k in persos.keys():
                    for i in persos[k]:
                        if persos[k][i].montype == "ballista":
                            dir = "D"
                            if 0 <= angquad <= 89:
                                dir = "D"
                            elif -90 <= angquad <= -1:
...
class Ouvrier(Perso):
    def __init__(self, parent, id, maison, couleur, x, y, montype):
        Perso.__init__(self, parent, id, maison, couleur, x, y, montype)
        self.vmax = 30 + self.parent.statistique["vie"] # vie initiale
        self.v = self.vmax  # vie courante
        self.bouclier = 0 + self.parent.statistique["defense"]
        self.b = self.bouclier  # bouclier courant
        self.attaque = 1  # l'attaque de l'ouvrier ne change pas
        self.a = self.attaque  # attaque courante
        self.idregion = None
        self.couleur = couleur[0]
        self.x = x
    ...
    def retour_batiment_mere(self):
        ...
        if reponse == "rendu":
            if self.cible:
                if self.typeressource == "baie":
                    self.parent.ressources["crypto"] += self.ramassage
                elif self.typeressource == "daim":
                    self.parent.ressources["batterie"] += self.ramassage
                elif self.typeressource == "troupeEnnemis" :
                    self.parent.ressources["gem"] += self.ramassage
                    self.q5 = True
                elif self.typeressource == "boss":
                    self.parent.ressources["gem"] += self.ramassage
                else:
    ...
    def valider_javelot(self):
        #while self.cible.v > 0:
        self.lancer_javelot(self.cible)
        for i in self.javelots:
            self.cible.v = i.bouger()
    ...
    def chercher_nouvelle_ressource(self, typ, idreg):
        if typ != "baie" and  typ != "daim" and typ != "troupeEnnemis" and typ != "boss":
            ...
            if typ == "troupeEnnemis":
                nb = len(self.parent.parent.troupeEnnemis[typ])
                for i in range(nb):
                    rep = random.choice(list(self.parent.parent.troupeEnnemis[typ].keys()))
                    obj = self.parent.parent.troupeEnnemis[typ][rep]
                    if obj != self.cible:
                ...
                return None
            if typ == "boss":
                #for i in self.parent.parent.npcs:
...
class Joueur():
    classespersos = {"ouvrier": Ouvrier,
                     ...
                     "ballista": Ballista,
                     "ingenieur": Ingenieur}
    ressources = {"Azteque": {"batterie": 999,
                              "arbre": 200,
                              "roche": 200,
                              "crypto": 200},
                  "Congolaise": {"batterie": 10,
                                 "arbre": 200,
                                 "roche": 200,
                                 "crypto": 888888888},
                  }
    ...
    def __init__(self, parent, id, nom, couleur, x, y):
        ...
        self.actions = {"creerperso": self.creer_perso,
                        ...
                        "acheter_potion" : self.acheter_potion}
        ...
        for i in troupe:
            for j in self.persos.keys():
                if i in self.persos[j]:
                    ...
                    elif typeress == "troupeEnnemis":
                        self.persos[j][i].chasser_ramasser(self.parent.troupeEnnemis[typeress][idress], typeress, "ciblerproie")
                    elif typeress == "boss":
                        for b in self.parent.npcs:
                            if b.id == idress:
                                self.persos[j][i].chasser_ramasser(b, typeress, "ciblerproie")
    ...
    def ramasser_ressource(self, param):
        ...
        for i in troupe:
            for j in self.persos.keys():
                if j == "ouvrier":
                    if typeress == "troupeEnnemis":
                        if i in self.persos[j]:
                            self.persos[j][i].chasser_ramasser(self.parent.troupeEnnemis[typeress][idress],
    ...
    def creer_point_origine(self, x, y):
        ...
        #if 1700 <= x <= 2300 and 2200 <= y <= 2800:
        self.batiments["maison"][idmaison] = Maison(self, idmaison, self.couleur, x, y, "maison")

    def acheter_potion(self, param):
        nomPotion = param[0]
        valsMarchand = Partie.valeursMarchand

        for k, vals in self.ressources.items():
            self.ressources[k] = vals - valsMarchand[nomPotion][k]

        if nomPotion == "vie1":
            # incrémenter le coût pour le prochain achat
            valsMarchand[nomPotion]["gem"] += 30
            # incrémenter la valeur pour l'affichage du bonus dans le HUD
            self.statistique["vie"] += 10

        if nomPotion == "attaque1":
            valsMarchand[nomPotion]["gem"] += 30
            self.statistique["attaque"] += 10

        if nomPotion == "defense1":
            valsMarchand[nomPotion]["gem"] += 30
            self.statistique["defense"] += 10

        if nomPotion == "vie2":
            valsMarchand[nomPotion]["gem"] += 30
            self.statistique["vie"] += 20

        if nomPotion == "attaque2":
            valsMarchand[nomPotion]["gem"] += 30
            self.statistique["attaque"] += 20

        if nomPotion == "defense2":
            valsMarchand[nomPotion]["gem"] += 30
            self.statistique["defense"] += 20
...
class Partie():
    ...
    def __init__(self, parent, mondict):
        ...
        self.regions = {}
        self.regionstypes = [["arbre", 20, 10, 5, "forest green"],
                             ["eau", 10, 10, 5, "light blue"],
                             ["marais", 5, 2, 1, "DarkSeaGreen3"],
                             ["roche", 8, 3, 6, "gray60"],
                             ["crypto", 8, 3, 4, "gold2"],]

        self.npcs = [] #comprend marchand, boss et coffre
        self.troupeEnnemis = {"troupeEnnemis" : {}}
        self.creer_regions()
        self.creer_biotopes()
        self.creer_npcs()
        self.creer_population(mondict)
        self.coutmarchand = 100

    def creer_npcs(self):
        # Marchand et boss
        id = get_prochain_id()
        m = Marchand(self, id, "marchand")
        self.marchand = m
        id = get_prochain_id()
        b = Boss(self, id, "boss")
        self.boss = b
    ...
    def ajouter_coffre(self):
        c = Coffre(self)
        c.x = random.randrange(100, self.aireX)
        c.y = random.randrange(100, self.aireY)
        self.npcs.append(c)

    def ajouter_pnj(self):
        me = MarchandEvent(self)
        me.x = random.randrange(100, self.aireX)
        me.y = random.randrange(100, self.aireY)
        me.etat = "arrivé"
        nombre = random.randrange(0, 4)
        me.cout = self.coutmarchand
        if nombre == 0:
            me.ressource = "batterie"
        elif nombre == 1:
            me.ressource = "bois"
        elif nombre == 2:
            me.ressource = "roche"
            ...
            me.ressource = "crypto"
        self.npcs.append(me)
        self.coutmarchand += 50

    def creer_troupeEnnemis(self):
        n = 0
        for i in self.npcs:
        ...
        if n <= 20:
            ...
            if case.montype == "plaine":
                id = get_prochain_id()
                matroupeEnnemis = TroupeEnnemis(self, id, x, y)
                self.troupeEnnemis["troupeEnnemis"][id] = matroupeEnnemis
                self.npcs.append(matroupeEnnemis)
    ...
    def trouver_valeursMarchand(self):
        vals = Partie.valeursMarchand
        return vals
    ...
    def creer_biotopes(self):
        ...
        self.creer_biotope("crypto", "crypto", Aureus)
    ...
    def creer_biotope(self, region, ressource, typeclasse):  # creation des forets
        ...
        for cleregion in self.regions[region].keys():
            ...
            nressource = int(random.randrange(len(listecases)) ) ## changement ici
            while nressource:
    ...
    def jouer_prochain_coup(self, cadrecourant):
        ...
        for i in self.troupeEnnemis["troupeEnnemis"].keys():
            self.troupeEnnemis["troupeEnnemis"][i].jouer_prochain_coup()
    ...
    def eliminer_ressource(self, type, ress):
        ...
        if type == "troupeEnnemis":
            if ress.id in self.troupeEnnemis[type]:
                self.troupeEnnemis[type].pop(ress.id)

        elif type == "boss":
            for b in self.npcs:
                if b.id == ress.id:
                    self.npcs.remove(b)
                    messagebox.showinfo(title="Félicitation !!!", message="Vous avez vaincu le boss !")

        elif type == "ouvrier" or type == "soldat" or type == "archer" or type == "chevalier" or type == "druide" or type == "ingenieur" or type == "ballista":
            for j in self.joueurs.keys():
                if ress.id in self.joueurs[j].persos[type]:
                    self.joueurs[j].persos[type].pop(ress.id)
