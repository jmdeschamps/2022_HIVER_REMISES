...
class Vue():
    def __init__(self,parent,urlserveur,monnom,testdispo):
        ...
        self.achatPotion = False

        self.nbtempBitcoin = 200
        self.nbBitcoin = 0
        self.coutmarchandevent = 100
        self.coutmarchandtext = 100
        self.ressourceevent = ""

        self.dansZoneBoss = False
...
####### INTERFACES GRAPHIQUES
    ...
    def creer_cadre_splash(self, urlserveur: str, monnom: str, testdispo: str) -> Frame:
        ...
        self.bgSplash = PhotoImage(file="images/bg/bgSplash.png")
        self.canevassplash = Canvas(self.cadresplash, width=600, height=480)
        self.canevassplash.create_image(-250, 0, anchor=NW, image=self.bgSplash)
    ...
    def creer_cadre_lobby(self):
        ...
        self.bgLobby = PhotoImage(file="images/bg/bgLobby.png")
        self.canevaslobby = Canvas(self.cadrelobby, width=600, height=480)
        self.canevaslobby.create_image(0, 0, anchor=NW, image=self.bgLobby)
    ...
    def creer_cadre_jeu(self):
        ...
        self.cadrepartie=Frame(self.cadreapp,bg="gray20",width=400,height=400)
    ...
    def creer_aire_de_jeu(self):
        ...
        self.canevas=Canvas(self.cadrecanevas,width=400,height=400,bg="gray20",
        ...
        self.canevas.create_oval(1600, 2100, 2400, 2900, disabledoutline="#990000", state="disabled")
    ...
    def creer_HUD(self):
        self.cadrejeuinfo=Frame(self.cadrecanevas)

        #des etiquettes d'info
        self.infohud={"Batteries":None,
                      "Bois":None,
                      "Roches":None,
                      "Cryptos":None,
                      "Gems":None,
        ...
        def creer_champ_interne2(listechamp):
            titre1=Champ(self.cadrejeuinfo, text=i,bg="red",fg="white")
            varstr1=StringVar()
            varstr1.set(0)
            donnee1=Champ(self.cadrejeuinfo,bg="red",fg="white", textvariable=varstr1)
            donnee1.pack(side=RIGHT)
            titre1.pack(side=RIGHT)
            self.infohudStats[i]=[varstr1,donnee1]
        ...
        for i in self.infohudStats.keys():
            creer_champ_interne2(i)
        varstr=StringVar()
        varstr1 = StringVar()
        varstr.set("")
        varstr1.set("")
        ...
        self.btnquetes=Button(self.cadrejeuinfo, text="Quêtes", command=self.action.quete)
        ...
        self.btnquetes.pack(side=RIGHT)
    ...
    def creer_cadre_jeu_action(self):
        ...
        self.canevasaction=Canvas(self.cadreaction,width=200,height=300,bg="cyan3",
        ...
        # minicarte
        self.minicarte=Canvas(self.cadreaction,width=self.tailleminicarte,height=self.tailleminicarte,bg="gray30",highlightthickness=0)
    ...
    def connecter_event(self):
        ...
        self.canevas.tag_bind("marchand", "<Button-1>", self.ajouter_selection)
        self.canevas.tag_bind("arbre", "<Button-1>", self.ramasser_ressource)
        self.canevas.tag_bind("crypto", "<Button-1>", self.ramasser_ressource)
        ...
        self.canevas.tag_bind("troupeEnnemis", "<Button-1>", self.chasser_ressource)
        self.canevas.tag_bind("boss", "<Button-1>", self.chasser_ressource)
    ...
    ### cadre qui s'Affichera par-dessus le canevas de jeu pour les quetes
    def creer_quetes(self):
        self.cadrequetes=Frame(self.canevas, bd=2, bg="magenta")
        self.scrollVquetes = Scrollbar(self.cadrequetes, orient=VERTICAL)
        self.textquetes = Text(self.cadrequetes, width=77, height=13,
                             yscrollcommand=self.scrollVquetes.set)
        self.scrollVquetes.config(command=self.textquetes.yview)
        self.textquetes.pack(side=LEFT)
        self.scrollVquetes.pack(side=LEFT, expand=1, fill=Y)
        #fichierquetes = open("quetes.txt")
        #mesquetes = fichierquetes.read()
        #fichierquetes.close()
        nbBatterie = self.modele.joueurs[self.parent.monnom].ressources["batterie"]
        nbBatiments = self.modele.joueurs[self.parent.monnom].q2
        nbDronesAttaque = 0

        if self.modele.joueurs[self.parent.monnom].ressources["crypto"] > self.nbtempBitcoin:
            self.nbBitcoin += self.modele.joueurs[self.parent.monnom].ressources["crypto"] - self.nbtempBitcoin
            self.nbtempBitcoin = self.modele.joueurs[self.parent.monnom].ressources["crypto"]
        elif self.modele.joueurs[self.parent.monnom].ressources["crypto"] < self.nbtempBitcoin:
            self.nbtempBitcoin = self.modele.joueurs[self.parent.monnom].ressources["crypto"]

        if self.modele.joueurs[self.parent.monnom].persos["ballista"]:
            nbDronesAttaque = 1
            if self.modele.joueurs[self.parent.monnom].q3:
                nbDronesAttaque = 2

        persos = self.modele.joueurs[self.parent.monnom].persos
        for k in persos.keys():
            for i in persos[k]:
                if persos[k][i].montype == "ouvrier":
                    if persos[k][i].q5:
                        self.q5done = True

        with open('quetes.txt', mode = 'r') as csv_file:
            csv_reader = csv.DictReader(csv_file)
            line_count = 0
            for row in csv_reader:
                self.textquetes.insert(END, f'\nQuete #{row["number"]} {row["title"]}')

                if line_count == 0 and nbBatterie >= 300:
                    self.q1done = True

                if self.q1done and line_count == 0:
                    self.textquetes.insert(END, f'\nProgression : {"D O N E !"}')

                elif (line_count == 0 and not self.q1done):
                    self.textquetes.insert(END, f'\nProgression : {nbBatterie}')

                if line_count == 1 and nbBatiments >= 1:
                    self.q2done = True

                if self.q2done and line_count == 1:
                    self.textquetes.insert(END, f'\nProgression : {"D O N E !"}')

                elif (line_count == 1 and not self.q2done):
                    self.textquetes.insert(END, f'\nProgression : {nbBatiments}')

                if line_count == 2 and nbDronesAttaque >= 2:
                    self.q3done = True

                if self.q3done and line_count == 2:
                    self.textquetes.insert(END, f'\nProgression : {"D O N E !"}')

                elif (line_count == 2 and not self.q3done):
                    self.textquetes.insert(END, f'\nProgression : {nbDronesAttaque} / 2')


                if (line_count == 3 and self.nbBitcoin >= 100):
                    self.q4done = True

                if self.q4done and line_count == 3:
                    self.textquetes.insert(END, f'\nProgression : {"D O N E !"}')

                elif (line_count == 3 and not self.q4done):
                    self.textquetes.insert(END, f'\nProgression : {self.nbBitcoin}')


                if (line_count == 4 and not self.q5done):
                    self.textquetes.insert(END, f'\nProgression : {"À  F A I R E"}')

                elif (line_count == 4 and self.q5done):
                    self.textquetes.insert(END, f'\nProgression : {"D O N E !"}')

                if (line_count == 5 and not self.q6done):
                    self.textquetes.insert(END, f'\nProgression : {"À  F A I R E"}')

                elif (line_count == 5 and self.q6done):
                    self.textquetes.insert(END, f'\nProgression : {"D O N E !"}')


                line_count += 1

        self.textquetes.config(state=DISABLED)

    ### cadre qui s'Affichera par-dessus le canevas de jeu pour l'aide
    def creer_aide(self):
        self.cadreaide=Frame(self.canevas, bd=2, bg="yellow")
...
##### FONCTIONS DU SPLASH #########################################################################
    ...
    def initialiser_avec_modele(self,modele):
        ...
        self.cadrejeuinfo.config(bg="cyan3")
        self.creer_aide()
        self.creer_cadre_marchand(["vie1","vie2","attaque1","attaque2","defense1","defense2"])
    ...
    def creer_cadre_marchand(self,artefacts):
        self.cadremarchand=Frame(self.canevasaction,bg ="white")
        for i in artefacts:

            var = "Gain :"
            varstr1=StringVar()
            varstr1.set(self.modele.joueurs[self.monnom].potions[i])
            btn=Button(self.cadremarchand,text=i,image=self.images[i], bg="white")
            label = Label(self.cadremarchand, text=i, font="12", bg="white")
            label2 = Label(self.cadremarchand, text=var , font="12")
            label1 = Label(self.cadremarchand, textvariable = varstr1, font="12", bg="white")


            btn.bind("<Button>",self.vendre_artefact)
            btn.pack()
            label.pack()
            label2.pack()
            label1.pack()
    ...
    def afficher_depart(self):
        ...
        for i in self.modele.npcs:

            if i.montype == "troupeEnnemis":
                self.canevas.create_image(i.x,i.y, image=self.images[i.img], tags=("mobile", "npc", i.id, i.montype))
    ...
    def afficher_batiment(self,joueur,batiment):
        ...
        #while (x0 > 1700 and x2 < 2300) and (y0 > 2200 and y2 < 2800):
    ...
    def afficher_jeu(self):
        ...
        for j in self.modele.joueurs.keys():
            ...
            if j==self.parent.monnom:
                self.infohud["Batteries"][0].set(self.modele.joueurs[j].ressources["batterie"])
                self.infohud["Bois"][0].set(self.modele.joueurs[j].ressources["arbre"])
                self.infohud["Roches"][0].set(self.modele.joueurs[j].ressources["roche"])
                self.infohud["Cryptos"][0].set(self.modele.joueurs[j].ressources["crypto"])
                self.infohud["Gems"][0].set(self.modele.joueurs[j].ressources["gem"])

                self.infohudStats["Vie"][0].set(self.modele.joueurs[j].statistique["vie"])
                self.infohudStats["Attaque"][0].set(self.modele.joueurs[j].statistique["attaque"])
                self.infohudStats["Defense"][0].set(self.modele.joueurs[j].statistique["defense"])
            ...
            for p in self.modele.joueurs[j].batiments['siteconstruction']:
                ...
                if s.etat=="attente":
                    ...
                    for h in self.modele.joueurs[j].persos.keys():
                        for k in self.modele.joueurs[j].persos[h].keys():
                            if self.modele.joueurs[j].persos[h][k].etat=="mort":
                                self.enleverX = True
                else:
        ...
        # Boss
        for b in self.modele.npcs:
            if b.montype == "boss":
                # si mort
                if b.etat == "mort":
                    self.canevas.delete(b.id)
                    self.canevas.create_image(b.x, b.y, image=self.images["bossMORT"],
                                              tags=("statique", "npc", b.id, b.montype))
                elif b.etat != "vivant":
                    self.canevas.delete(b.id)
                # projectile Boss
                for t in b.javelots:
                    self.canevas.create_image(t.x, t.y, image=self.images[t.image],
                                              tags=("mobile", b, t.id, "", type(t).__name__, ""))

        # troupes ennemis
        for j in self.modele.troupeEnnemis["troupeEnnemis"].keys():
            i = self.modele.troupeEnnemis["troupeEnnemis"][j]
            if i.etat == "mort":
                for l in i.javelots:
                    i.javelots.remove(l)
                self.canevas.create_image(i.x, i.y, image=self.images["troupeEnnemisMORT"],
                                          tags=("mobile", "npc", i.id, i.montype))
            else :
                self.canevas.create_image(i.x, i.y, image=self.images[i.img],
                                          tags=("mobile", "npc", i.id, i.montype))

            # dessiner projectiles de troupe ennemis
            for t in i.javelots:
                self.canevas.create_image(t.x, t.y, image=self.images[t.image],
                                          tags=("mobile", j, t.id, "", type(t).__name__, ""))

        # ajuster les choses vivantes dependantes de la partie (mais pas des joueurs)
        for j in self.modele.biotopes["baie"].keys():
            i=self.modele.biotopes["baie"][j]
            if i.sprite:
        ...
        if i.sprite:
            self.canevas.create_image(i.x,i.y,image=self.gifs[i.sprite][i.spriteno],
                                        tags=("mobile","npcs",i.id,"boss",i.montype))

        i = self.modele.marchand
        if i.sprite:
            self.canevas.create_image(i.x, i.y, image=self.gifs["marchand"][i.spriteno],
                                      tags=("mobile", "", i.id, "marchand", i.montype))
        ...
        # Quand on a un ouvrier de sélectionné et qu'il meurt, il faut le déselectionner
        persos = self.modele.joueurs[self.parent.monnom].persos
        for k in persos.keys():
            for i in persos[k]:
                for l in self.action.persochoisi:
                    if l == persos[k][i].id:
                        if persos[k][i].etat == "mort":
                            self.action.persochoisi.remove(l)
...
##### ACTIONS DU JOUEUR #######################################################################
    ...
    def ajouter_selection(self,evt):
        ...
        if self.parent.monnom == mestags[1]:
            ...
            if "marchand" == mestags[3]:
                self.action.persochoisi = []
                self.action.afficher_commande_marchand()
    ...
    def ramasser_ressource(self,evt):
        print("dans ramasser_ressource 1 de la vue")
        tag=self.canevas.gettags(CURRENT)
        if tag[1]=="" and self.action.persochoisi:
            self.action.ramasser_ressource(tag)
        elif tag[1] == "npc" and self.action.persochoisi and tag[3] == "troupeEnnemis":
    ...
    def chasser_ressource(self,evt):
        ...
        if tag[1]=="" and self.action.persochoisi and tag[4]=="daim":
            ...
            self.action.chasser_ressource(tag)
        elif self.action.persochoisi and tag[3] == "boss":
            self.action.chasser_ressource(tag)
        else:
            print(tag[1])
            print(tag[3])
    ...
    def verifier_proche_coffre(self):
        persos = self.modele.joueurs[self.parent.monnom].persos
        for k in persos.keys():
            for i in persos[k]:
                for j in self.modele.npcs:
                    if j.montype == "coffre":
                        if (j.x + 100 >= persos[k][i].x >= j.x - 100) and (j.y + 100 >= persos[k][i].y >= j.y - 100):
                            self.modele.joueurs[self.parent.monnom].ressources["gem"] += 100
                            self.canevas.delete(j.id)
                            self.modele.npcs.remove(j)

    def verifier_proche_pnj(self):
        persos = self.modele.joueurs[self.parent.monnom].persos
        for k in persos.keys():
            for i in persos[k]:
                for j in self.modele.npcs:
                    if j.montype == "marchandevent":
                        if (j.x + 100 >= persos[k][i].x >= j.x - 100) and (j.y + 100 >= persos[k][i].y >= j.y - 100):
                            if self.modele.joueurs[self.parent.monnom].ressources[str(j.ressource)] >= j.cout:
                                self.modele.joueurs[self.parent.monnom].ressources["gem"] += self.coutmarchandevent
                                self.canevas.delete(j.id)
                                self.modele.npcs.remove(j)
                                self.modele.joueurs[self.parent.monnom].ressources[str(j.ressource)] -= j.cout
                                self.modele.joueurs[self.parent.monnom].ressources["gem"] = random.randrange(j.cout - 50, j.cout + 50)

    def event(self):
        nombre = random.randrange(0, 2)
        if nombre == 0:
            self.modele.ajouter_coffre()
            for i in self.modele.npcs:
                if i.montype == "coffre":
                    self.canevas.create_image(i.x, i.y, image=self.images["coffre"], tags=("statique","npc", i.id, i.montype))
            messagebox.showinfo(title="E V E N T !", message="Un largage est arrivé, cherchez-le pour une récompense !")
        if nombre == 1:
            for i in self.modele.npcs:
                if i.montype == "marchandevent":
                    i.etat = "encore là"
            self.modele.ajouter_pnj()
            for i in self.modele.npcs:
                if i.montype == "marchandevent":
                    self.canevas.create_image(i.x, i.y, image=self.images["minecraft_npc"], tags=("statique", "npc", i.id, i.montype))
                    messagebox.showinfo(title="E V E N T !", message="Un marchand mystérieux est " + i.etat + ", apporter lui " + str(i.cout) + " " + i.ressource + " pour des gems !")

    def creer_barrevie(self):
        for i in self.modele.npcs:
            if i.montype == "troupeEnnemis":
                if i.etat == "mort" or i.v == 0:
                    self.canevas.delete(i.fondVie, i.rectVie)
                else:
                    if i.fondVie == None:
                        i.fondVie = self.canevas.create_rectangle(i.x-50, i.y-50, i.x+50, i.y-40, disabledoutline="#606060", disabledfill="#CC0000", state="disabled")
                        i.rectVie = self.canevas.create_rectangle(i.x-49, i.y-49, (i.vmax/i.v)*49, i.y-41, disabledfill="#00CC00", state="disabled")
                    else:
                        if i.v != 0:
                            self.canevas.delete(i.fondVie, i.rectVie)
                            i.fondVie = self.canevas.create_rectangle(i.x - 50, i.y - 50, i.x + 50, i.y - 40,disabledoutline="#606060", disabledfill="#CC0000", state="disabled")
                            i.rectVie = self.canevas.create_rectangle(i.x - 49, i.y - 49, i.x-49 + (98/(i.vmax / i.v)), i.y - 41, disabledfill="#00CC00", state="disabled")
            elif i.montype == "boss":
                if i.etat == "mort":
                    self.canevas.delete(i.fondVie, i.rectVie)
                else:
                    if i.fondVie == None:
                        i.fondVie = self.canevas.create_rectangle(i.x - 150, i.y - 150, i.x + 150, i.y - 130,disabledoutline="#606060", disabledfill="#CC0000",state="disabled")
                        i.rectVie = self.canevas.create_rectangle(i.x - 149, i.y - 149, i.x - 149 + (298 / (i.vmax / i.v)),i.y - 131, disabledfill="#00CC00", state="disabled")
                    else:
                        if i.v > 0:
                            self.canevas.delete(i.fondVie, i.rectVie)
                            i.fondVie = self.canevas.create_rectangle(i.x - 150, i.y - 150, i.x + 150, i.y - 130,disabledoutline="#606060", disabledfill="#CC0000", state="disabled")
        ...
        for k in persos.keys():
            for i in persos[k]:
                if persos[k][i].v != 0:
                    if persos[k][i].fondVie == None:
                        persos[k][i].fondVie = self.canevas.create_rectangle(persos[k][i].x - 30, persos[k][i].y - 60, persos[k][i].x + 30, persos[k][i].y - 55, disabledoutline="#606060", disabledfill="#CC0000", state="disabled")
                        persos[k][i].rectVie = self.canevas.create_rectangle(persos[k][i].x - 30, persos[k][i].y - 59, persos[k][i].x - 29 + (58/(persos[k][i].vmax / persos[k][i].v)), persos[k][i].y - 56, disabledfill="#00CC00",state="disabled")
                    else:
                        self.canevas.delete(persos[k][i].fondVie, persos[k][i].rectVie)
                        persos[k][i].fondVie = self.canevas.create_rectangle(persos[k][i].x - 30, persos[k][i].y - 60, persos[k][i].x + 30, persos[k][i].y - 55, disabledoutline="#606060", disabledfill="#CC0000", state="disabled")
                        persos[k][i].rectVie = self.canevas.create_rectangle(persos[k][i].x - 30, persos[k][i].y - 59, persos[k][i].x - 29 + (58 / (persos[k][i].vmax / persos[k][i].v)), persos[k][i].y - 56, disabledfill="#00CC00",state="disabled")
                    if persos[k][i].b > 0:
                        if persos[k][i].fondBouc == None:
                            persos[k][i].fondBouc = self.canevas.create_rectangle(persos[k][i].x - 30,persos[k][i].y - 60,persos[k][i].x + 30,persos[k][i].y - 55,disabledoutline="#606060",disabledfill="#00CC00",state="disabled")
                            persos[k][i].rectBouc = self.canevas.create_rectangle(persos[k][i].x - 30,persos[k][i].y - 59,persos[k][i].x - 29 + (58 / (persos[k][i].bouclier /persos[k][i].b)),persos[k][i].y - 56,disabledoutline="#606060",disabledfill="#ADD8E6",state="disabled")
                        else:
                            self.canevas.delete(persos[k][i].fondBouc, persos[k][i].rectBouc)
                            persos[k][i].fondBouc = self.canevas.create_rectangle(persos[k][i].x - 30,persos[k][i].y - 60,persos[k][i].x + 30,persos[k][i].y - 55,disabledoutline="#606060",disabledfill="#00CC00",state="disabled")
                            persos[k][i].rectBouc = self.canevas.create_rectangle(persos[k][i].x - 30,persos[k][i].y - 59,persos[k][i].x - 29 + (58 / (persos[k][i].bouclier /persos[k][i].b)),persos[k][i].y - 56,disabledoutline="#606060",disabledfill="#ADD8E6",state="disabled")
                    elif persos[k][i].b <= 0:
                        self.canevas.delete(persos[k][i].fondBouc, persos[k][i].rectBouc)
    ...
    def vendre_artefact(self, evt):

        obj = evt.widget
        nomPotion = obj.cget("text")
        message = messagebox.askokcancel(title="Le marchand vous parle :", message="La potion " + nomPotion + " vous permettra de gagner plus de combats \n Etes vous certains de vouloir acheter cette potion ?")

            # on valide qu'on a assez de ressources pour construire
        valsmarchand = self.parent.trouver_valeursMarchand()

        ok = 1

        for k, val in self.modele.joueurs[self.monnom].ressources.items():
            if k == "gem":
                if val <= valsmarchand[nomPotion][k]:
        ...
        if message:
            if ok:
                action = [self.monnom, "acheter_potion", [nomPotion]]
                self.parent.actionsrequises.append(action)
                self.q6done = True
            else :
                messagebox.showinfo(title="Vous êtes pauvre !", message="Nombre de gemmes insuffisant")
        else:
            messagebox.showinfo(title="le marchand vous parle :", message="Ok une prochaine fois peut être")
    ...
    def batir_artefact(self,evt):
        ...
        for k,val in self.modele.joueurs[self.monnom].ressources.items():
            if val < vals[nomsorte][k]:
                if k == "bois" or k == "roche" or k == "batterie":
                    ok=0 # on indique qu'on a PAS les ressources
                    break
        if ok:
            ...
            self.action.btnactif.config(bg="SystemButtonFace")
            messagebox.showinfo(title="Bruh !", message="Nombre de ressources insuffisant")

    def construire_batiment(self,evt):
        mestags=self.canevas.gettags(CURRENT)
        if not mestags and self.action.persochoisi and self.action.prochaineaction:
            if len(self.action.persochoisi) > 1:
                persotemp = self.action.persochoisi[0]
                while len(self.action.persochoisi) > 0:
                    for i in self.action.persochoisi:
                        self.action.persochoisi.remove(i)
                self.action.persochoisi.append(persotemp)
        ...
        if self.parent.monnom in mestags:
            if "batiment" in mestags:
                if "maison" in mestags:
                    persos = self.modele.joueurs[self.parent.monnom].persos
                    for i in persos["ouvrier"]:
                        self.nbOuvrier += 1

                    if self.modele.joueurs[self.parent.monnom].ressources["crypto"] >= 10:
                        pos=(self.canevas.canvasx(evt.x),self.canevas.canvasy(evt.y))
                        action=[self.parent.monnom,"creerperso",["ouvrier",mestags[4],mestags[2],pos]]
                        self.modele.joueurs[self.parent.monnom].ressources["crypto"] -= 10
                        self.parent.actionsrequises.append(action)
                        self.nbOuvrier += 1
                    else:
                        if self.nbOuvrier == 0:
                ...
                if "caserne" in mestags:
                    if self.modele.joueurs[self.parent.monnom].ressources["crypto"] >= 25:
                        if self.modele.joueurs[self.parent.monnom].ressources["roche"] >= 10:
                            pos=(self.canevas.canvasx(evt.x),self.canevas.canvasy(evt.y))
                            action=[self.parent.monnom,"creerperso",["soldat",mestags[4],mestags[2],pos]]
                            self.parent.actionsrequises.append(action)
                            self.modele.joueurs[self.parent.monnom].ressources["crypto"] -= 25
                            self.modele.joueurs[self.parent.monnom].ressources["roche"] -= 10

                if "abri" in mestags:
                    if self.modele.joueurs[self.parent.monnom].ressources["crypto"] >= 15:
                        if self.modele.joueurs[self.parent.monnom].ressources["bois"] >= 10:
                            pos=(self.canevas.canvasx(evt.x),self.canevas.canvasy(evt.y))
                            action=[self.parent.monnom,"creerperso",["druide",mestags[4],mestags[2],pos]]
                            self.parent.actionsrequises.append(action)
                            self.modele.joueurs[self.parent.monnom].ressources["crypto"] -= 15
                            self.modele.joueurs[self.parent.monnom].ressources["bois"] -= 10

                if "usineballiste" in mestags:
                    if self.modele.joueurs[self.parent.monnom].ressources["crypto"] >= 50:
                        if self.modele.joueurs[self.parent.monnom].ressources["batterie"] >= 20:
                            pos=(self.canevas.canvasx(evt.x),self.canevas.canvasy(evt.y))
                            action=[self.parent.monnom,"creerperso",["ballista",mestags[4],mestags[2],pos]]
                            self.parent.actionsrequises.append(action)
                            self.modele.joueurs[self.parent.monnom].ressources["crypto"] -= 50
                            self.modele.joueurs[self.parent.monnom].ressources["batterie"] -= 20
...
class Action():
    ...
    def chasser_ressource(self,tag):
        if self.persochoisi:
            if tag[3] == "troupeEnnemis":
                action = [self.parent.parent.monnom, "chasserressource", [tag[3], tag[2], self.persochoisi]]
                self.parent.parent.actionsrequises.append(action)
            elif tag[3] == "boss":
                action = [self.parent.parent.monnom, "chasserressource", [tag[3], tag[2], self.persochoisi]]
                self.parent.parent.actionsrequises.append(action)
                print("chasse le boss")
            else :
    ...
    def ramasser_ressource(self,tag):
        if self.persochoisi:
            if tag[3] == "troupeEnnemis":
                action = [self.parent.parent.monnom, "ramasserressource", [tag[3], tag[2], self.persochoisi]]
                self.parent.parent.actionsrequises.append(action)
            else :
    ...
    def afficher_commande_marchand(self):
        self.widgetsactifs=self.parent.canevasaction.create_window(100,60,
                                                window=self.parent.cadremarchand,
                                                anchor=N)
        self.parent.root.update()
        fh=self.parent.cadremarchand.winfo_height()
    ...
    def quete(self):
        if self.queteon == 0:
            ...
            y1, y2 = self.parent.scrollV.get()
            y3 = self.parent.modele.aireY * y1
            self.queteon = self.parent.canevas.create_window(x3, y3,
                                                            window=self.parent.cadrequetes,
                                                            anchor=N + E)
        else:
            self.parent.canevas.delete(self.queteon)
            self.queteon = 0
