...
class MenuPrincipal:
    ...
    def __init__(self, view, urlserveur: str, monnom: str, testdispo: str):
        ...
        self.canevassplash = Canvas(self.cadresplash, width=595, height=475, bg="royal blue")
        self.canevassplash.pack()

        # creation ds divers widgets (champ de texte 'Entry' et boutons cliquables (Button)
        self.canevassplash.create_image(0,0, anchor=NW, image=self.images['menu_background'])
        self.etatdujeu = Label(text=testdispo, font=("Arial", 18), borderwidth=2, relief=RIDGE, bg="black", fg="white")
        self.nomsplash = Entry(font=("Arial", 14), bg="black", fg="white")
        self.urlsplash = Entry(font=("Arial", 14), bg="black", fg="white")
        self.btnurlconnect = Button(text="Connecter", font=("Arial", 12), command=self.connecter_serveur, bg="black", fg="white")
        ...
        self.canevassplash.create_image(300, 75, anchor=CENTER, image=self.images["Battle_for_Wesnoth_logo"])
        self.canevassplash.create_window(300, 150, window=self.etatdujeu, width=400, height=30)
        self.canevassplash.create_window(300, 200, window=self.nomsplash, width=400, height=30)
        self.canevassplash.create_window(220, 250, window=self.urlsplash, width=200, height=30)
        self.canevassplash.create_window(400, 250, window=self.btnurlconnect, width=100, height=30)
        # les boutons d'actions
        self.btncreerpartie = Button(text="Creer partie", font=("Arial", 12), state=DISABLED, command=self.creer_partie, bg="black", fg="white")
        self.btninscrirejoueur = Button(text="Inscrire joueur", font=("Arial", 12), state=DISABLED,
                                        command=self.inscrire_joueur, bg="black", fg="white")
        self.btnreset = Button(text="Reinitialiser partie", font=("Arial", 9), state=DISABLED,
                               command=self.reset_partie, bg="black", fg="white")

        # on place les autres boutons
        self.canevassplash.create_window(400, 325, window=self.btncreerpartie, width=200, height=30)
        self.canevassplash.create_window(400, 375, window=self.btninscrirejoueur, width=200, height=30)
        self.canevassplash.create_window(400, 425, window=self.btnreset, width=200, height=30)
