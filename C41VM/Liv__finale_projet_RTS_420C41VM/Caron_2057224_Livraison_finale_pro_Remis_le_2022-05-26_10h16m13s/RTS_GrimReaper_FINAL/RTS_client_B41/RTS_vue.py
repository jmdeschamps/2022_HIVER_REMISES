...
# import messagebox from tkinter module #ss
...
class Vue():
    def __init__(self, parent, urlserveur, monnom, testdispo):
        ...
        # attributs
        self.cadrechaton = 0
        self.cadrechoix = 0  ##ajout alex pour choix de retour
        ...
        self.cadreapp = Frame(self.root, width=500, height=400, bg=CADRE_15)
    ...
    def creer_cadre_splash(self, urlserveur: str, monnom: str, testdispo: str) -> Frame:
        ...
        self.canevassplash = Canvas(self.cadresplash, width=600, height=480, bg=CADRE_3)
        self.canevassplash.pack()

        # creation ds divers widgets (champ de texte 'Entry' et boutons cliquables (Button)
        self.etatdujeu = Label(text=testdispo, font=("Arial", 18), borderwidth=2, relief=RIDGE, bg=CADRE_6)
        self.nomsplash = Entry(font=("Arial", 14), bg=CADRE_6)
        self.urlsplash = Entry(font=("Arial", 14), bg=CADRE_6)
        self.btnurlconnect = Button(text="Connecter", font=("Arial", 12), command=self.connecter_serveur, bg=CADRE_6)
        ...
        self.btninscrirejoueur = Button(text="Inscrire joueur", font=("Arial", 12), state=DISABLED,
                                        command=self.inscrire_joueur, bg=CADRE_6)
        self.btnreset = Button(text="Reinitialiser partie", font=("Arial", 9), state=DISABLED,
                               command=self.reset_partie, bg=CADRE_6)
    ...
    def creer_cadre_lobby(self):
        ...
        self.canevaslobby = Canvas(self.cadrelobby, width=640, height=480, bg=CADRE_14)
        self.canevaslobby.pack()
        # widgets du lobby
        # un listbox pour afficher les joueurs inscrit pour la partie à lancer
        self.listelobby = Listbox(borderwidth=2, relief=GROOVE, bg=CADRE_10)
    ...
    def creer_cadre_jeu(self):
        ...
        self.cadrepartie = Frame(self.cadreapp, bg=CADRE_15, width=400, height=400)
    ...
    def creer_aire_de_jeu(self):
        ...
        #  pour modifier les paramètres du canevas de jeu
        self.canevas = Canvas(self.cadrecanevas, width=JEU_WIDTH, height=JEU_HEIGHT,
                              bg=TEMPS_COULEUR[0],
    ...
    def creer_HUD(self):
        self.cadrejeuinfo = Frame(self.cadrecanevas, bg=CADRE_15)
        # des etiquettes d'info
        self.infohud = {"Soul": None,  # ss
                        "Soul_usine": None,  # Alex
                        "Bois": None,
                        "Roche": None,
                        "Aureus": None,
                        "Grim": None,  # ss
                        ...
                        "Ingenieur": None,  # ss
                        PORTE: None,  # ss
                        ROGUEFACTORY: None,  # SS
                        KIDFACTORY: None,  # SS
                        SOULFARM: None}  # SS

        # fonction interne uniquement pour reproduire chaque info de ressource
        def creer_champ_interne(listechamp):
            titre = Champ(self.cadrejeuinfo, text=i, bg=CADRE_15, fg="white")
            varstr = StringVar()
            varstr.set(0)
            donnee = Champ(self.cadrejeuinfo, bg=CADRE_15, fg="white", textvariable=varstr)
    ...
    def creer_cadre_jeu_action(self):
        ...
        self.canevasaction = Canvas(self.cadreaction, width=200, height=300, bg=CADRE_5,
        ...
        # minicarte
        self.minicarte = Canvas(self.cadreaction, width=self.tailleminicarte, height=self.tailleminicarte, bg=CADRE_15,
                                highlightthickness=0)
        self.minicarte.grid(row=2, column=0, columnspan=2)
        self.minicarte.bind("<Button-1>", self.deplacer_carte)

        # message important (du genre c'est le jour, c'est la nuit)
        self.message_important = Frame(self.cadreaction, width=200, height=160, bg=CADRE_10, highlightthickness=0)
        self.message_important.grid(row=1, column=0, columnspan=1)
        self.textmessageimportant = Text(self.message_important, width=20, height=2)
        self.textmessageimportant.pack(side=TOP)

        # on retourne ce cadre pour l'insérer dans le dictionnaires des cadres
        self.canevasaction.rowconfigure(0, weight=1)
        self.cadreaction.rowconfigure(0, weight=1)

    def creer_cadre_win(self, nom):  # SS
        win = Tk()
        ...
        Label(win, text="Le joueur " + nom + " a gagné", font=('Helvetica 16 underline')).pack(pady=20)
        ttk.Button(win, text="Accepter", command=handler).pack(pady=30)
        # pour pauser le jeu quand on a gagné - à suivre
        # win.mainloop()
    ...
    def connecter_event(self):
        ...
        self.canevas.tag_bind("batiment", "<Button-1>", self.clic_batiment)
        ...
        self.canevas.tag_bind("grave", "<Button-1>", self.ramasser_ressource)
        self.canevas.tag_bind("humain", "<Button-1>", self.chasser_ressource)

        self.canevas.bind("<Control-Button-1>", self.parent.montrer_stats)

    # Sean attaquer batiment
    def clic_batiment(self, evt):
        mestags = self.canevas.gettags(CURRENT)
        if self.parent.monnom == mestags[1]:
            self.creer_entite(evt)
        else:
    ...
    ### Alex  cadre qui s'Affichera lorsque l'on veut choisir ou aller porter la ressources
    def choix_res(self):
        self.cadrechoix_red = Frame(self.canevas, bd=2, bg="orange")
        ##self.cadrechoix_red.pack(side=LEFT)
        self.btnusine = Button(self.cadrechoix_red, text="Usine", command=self.action.choix_usine)
        self.btnporte = Button(self.cadrechoix_red, text="Porte", command=self.action.choix_porte)
        self.btnusine.pack()
        self.btnporte.pack()
    ...
    def initialiser_avec_modele(self, modele):
        ...
        # cadre apparaissant si on selectionne un Grim
        coul = self.modele.joueurs[self.parent.monnom].couleur
        self.cadrejeuinfo.config(bg=coul[1])
        self.creer_aide()
        self.creer_cadre_grim(coul[0] + "_", [PORTE, KIDFACTORY, ROGUEFACTORY, SOULFARM])
        self.creer_chatter()
        self.choix_res()  ##ajout alex pour le choix de retour des ames
        # on affiche les portes, point de depart des divers joueurs
        self.afficher_depart()
        self.root.update()
        # self.centrer_porte()

    def creer_cadre_grim(self, coul, artefacts):
        self.cadregrim = Frame(self.canevasaction)
        for i in artefacts:
            btn = Button(self.cadregrim, text=i) # , image=self.images[coul + i]  YOAN
    ...
    def afficher_depart(self):
        ...
        px = self.tailleminicarte / self.modele.aireX
        py = self.tailleminicarte / self.modele.aireY

        for j in self.modele.joueurs.keys():
            for i in self.modele.joueurs[j].batiments[PORTE].keys():
                m = self.modele.joueurs[j].batiments[PORTE][i]
                coul = self.modele.joueurs[j].couleur[0]
                self.canevas.create_image(m.x, m.y, image=self.images[coul + "_porte"],
                ...
                self.minicarte.create_rectangle(x1 - 2, y1 - 2, x1 + 2, y1 + 2, fill=coul,
                                                tags=(j, m.id, "artefact", PORTE))
                for n in self.modele.joueurs[j].persos["grim"].keys():
                    p = self.modele.joueurs[j].persos["grim"][n]
                    pos = (self.canevas.canvasx(x1 + 3), self.canevas.canvasy(y1 + 3))
                    self.parent.action = [self.parent.monnom, "creerperso", ["grim", "porte", m.id, pos]]

            for p in self.modele.joueurs[j].batiments[
                SOULFARM].keys():  # création de l'usine ferme d'âme YOAN ALEX
                m = self.modele.joueurs[j].batiments[SOULFARM][p]
                coul = self.modele.joueurs[j].couleur[0]
                self.canevas.create_image(m.x, m.y, image=self.images[coul + "_soulFarm"],
                                          tags=("statique", j, m.id, "batiment", m.montype, ""))
                # tags=("mobile","",i.id,)
                # afficher sur minicarte
                coul = self.modele.joueurs[j].couleur[1]
                ...
                y1 = (m.y / self.modele.aireY) * self.tailleminicarte
                self.minicarte.create_rectangle(x1 - 2, y1 - 2, x1 + 2, y1 + 2, fill=coul,
                                                tags=(j, m.id, "artefact", SOULFARM))
                for n in self.modele.joueurs[j].persos["funeral"].keys():
                    p = self.modele.joueurs[j].persos["funeral"][n]
                    pos = (self.canevas.canvasx(x1 + 3), self.canevas.canvasy(y1 + 3))
                    self.parent.action = [self.parent.monnom, "creerperso", ["funeral", "soulFarm", m.id, pos]]
    ...
    def afficher_jeu(self):
        ...
        for i in self.modele.batimentmort:
            self.canevas.delete(i.id)
        for i in self.modele.graveempty:
            i.img = "grave_empty"
            self.canevas.delete(i.id)
            self.afficher_bio(i)
        ...
        for j in self.modele.joueurs.keys():
            ...
            if j == self.parent.monnom:
                self.infohud["Soul"][0].set(self.modele.joueurs[j].ressources["soul"])  # ss
                self.infohud["Soul_usine"][0].set(self.modele.joueurs[j].ressources["soul_usine"])  # Alex
                ...
                self.infohud["Grim"][0].set(self.modele.joueurs[j].ressources["grim"])  # ss
                self.infohud["kidReaper"][0].set(self.modele.joueurs[j].ressources["kidReaper"])  # ss
                self.infohud["RogueReaper"][0].set(self.modele.joueurs[j].ressources["RogueReaper"])  # ss
                self.infohud["Funeral"][0].set(self.modele.joueurs[j].ressources["funeral"])  # ss
                self.infohud["Ingenieur"][0].set(self.modele.joueurs[j].ressources["ingenieur"])  # ss
                self.infohud[PORTE][0].set(self.modele.joueurs[j].ressources[PORTE])  # ss
                self.infohud[ROGUEFACTORY][0].set(self.modele.joueurs[j].ressources[ROGUEFACTORY])  # ss
                self.infohud[KIDFACTORY][0].set(self.modele.joueurs[j].ressources[KIDFACTORY])  # ss
                self.infohud[SOULFARM][0].set(self.modele.joueurs[j].ressources[SOULFARM])  # ss
            ...
            # ajuster les persos de chaque joueur et leur dépendance (ici javelots des grims)
            for p in self.modele.joueurs[j].persos.keys():
                for k in self.modele.joueurs[j].persos[p].keys():
                    ...
                    # dessiner javelot de l'grim
                    if p == "RogueReaper":  # SS RogueReaper collecte ressources
        ...
        for j in self.modele.biotopes["humain"].keys():
            i = self.modele.biotopes["humain"][j]
            if i.etat == "mort":
                self.canevas.create_image(i.x, i.y, image=self.images["humainMORT"],
                                          tags=("mobile", "", i.id, "biotope", i.montype, ""))
                # tags=("",i.id,"artefact","humain","mobile"))
        ...
        # ajuster les choses vivantes dependantes de la partie (mais pas des joueurs)
        # for j in self.modele.biotopes["brume"].keys():
        #     i = self.modele.biotopes["brume"][j]
        #     if i.sprite:
        ...
        if self.textchat and self.modele.joueurs[self.parent.monnom].chatneuf:
            ...
            if self.modele.joueurs[self.parent.monnom].chatneuf and self.action.chaton == 0:
                self.btnchat.config(bg=CADRE_12)
            self.modele.joueurs[self.parent.monnom].chatneuf = 0

    def centrer_porte(self):
        self.root.update()
        cle = list(self.modele.joueurs[self.monnom].batiments[PORTE].keys())[0]
        x = self.modele.joueurs[self.monnom].batiments[PORTE][cle].x
        y = self.modele.joueurs[self.monnom].batiments[PORTE][cle].y
    ...
    def ajouter_selection(self, evt):
        ...
        if self.parent.monnom == mestags[1]:
            ...
            if "RogueReaper" == mestags[4]:  # SS RogueReaper ramasse ressources
                self.action.persochoisi.append(mestags[2])
                self.action.afficher_commande_perso()
            ######
            else:
    ...
    def chasser_ressource(self, evt):
        tag = self.canevas.gettags(CURRENT)
        print(tag)
        if tag[1] == "" and self.action.persochoisi and tag[4] == "humain":
    ...
    def batir_artefact(self, evt):
        ...
        for k, val in self.modele.joueurs[self.monnom].ressources.items():
            if val < vals[nomsorte][k]:  # SS (était <=, changé pour <, sinon on ne peut pas construire à 0 ressources)
                ok = 0  # on indique qu'on a PAS les ressources
                break
        if ok:
            self.action.prochaineaction = obj.cget("text")
            obj.config(bg=CADRE_12)
        else:
            self.action.btnactif.config(bg="SystemButtonFace")
    ...
    def creer_entite(self, evt):
        ...
        if self.parent.monnom in mestags:
            if "batiment" in mestags:
                if PORTE in mestags:
                    if self.parent.modele.joueurs[mestags[1]].ressources["grim"] == 0:
                        pos = (self.canevas.canvasx(evt.x), self.canevas.canvasy(evt.y))
                        action = [self.parent.monnom, "creerperso", ["grim", mestags[4], mestags[2], pos]]
                if KIDFACTORY in mestags:
                    pos = (self.canevas.canvasx(evt.x), self.canevas.canvasy(evt.y))
                    action = [self.parent.monnom, "creerperso", ["kidReaper", mestags[4], mestags[2], pos]]
                if ROGUEFACTORY in mestags:
                    pos = (self.canevas.canvasx(evt.x), self.canevas.canvasy(evt.y))
                    action = [self.parent.monnom, "creerperso", ["RogueReaper", mestags[4], mestags[2], pos]]
                if SOULFARM in mestags:
                    if self.parent.modele.joueurs[mestags[1]].ressources["funeral"] == 0:
                        pos = (self.canevas.canvasx(evt.x), self.canevas.canvasy(evt.y))
                        action = [self.parent.monnom, "creerperso", ["grim", mestags[4], mestags[2], pos]]
                    else:
                        action = [self.parent.monnom, "transferersoul", [mestags[2]]]
                        print("transfert")
                if action:
...
class Action():
    def __init__(self, parent):
        ...
        self.chaton = 0
        self.aideon = 0
        self.choixon = 0  ## alex pour le choix de retour

    def attaquer(self):
        if self.persochoisi:
            qui = self.ciblechoisi[1]
            cible = self.ciblechoisi[2]
            sorte = self.ciblechoisi[4] #SS attaquer batiment
    ...
    def chasser_ressource(self, tag):
        if self.persochoisi:
            ...
            print(tag[0])
            print(tag[1])  # tag vide
            print(tag[2])
            print(tag[3])
            print(tag[4])
            print(tag[5])  # tag vide
            self.parent.parent.actionsrequises.append(action)
    ...
    def afficher_commande_perso(self):
        self.widgetsactifs = self.parent.canevasaction.create_window(100, 60,
                                                                     window=self.parent.cadregrim,
                                                                     anchor=N)
        self.parent.root.update()
        fh = self.parent.cadregrim.winfo_height()
    ...
    ##Alex pour lw choix de retour des ames
    def choix(self):
        if self.choixon == 0:
            ...
            self.parent.cadrechoix = self.parent.canevas.create_window(x3, y3,
                                                                       window=self.parent.cadrechoix_red,
                                                                       anchor=N + W)
        else:
            self.parent.canevas.delete(self.parent.cadrechoix)
            self.parent.cadrechoix = 0
            self.choixon = 0
    ...
    def choix_usine(self):  ##Alex pour le retour a l'usine
        self.parent.parent.modele.on_off = 1
        ##self.choixon = 1
        self.parent.btnusine.config(bg="SystemButtonFace")
        self.parent.canevas.delete(self.parent.cadrechoix)

    def choix_porte(self):  ##Alex pour le retrou a l'usine
        self.parent.parent.modele.on_off = 2
        ##self.choixon = 1
        self.parent.btnporte.config(bg="SystemButtonFace")
        self.parent.canevas.delete(self.parent.cadrechoix)
