...
class Controleur():
    def __init__(self):
        ...
        self.vaisseau_vue = Vaisseau_vue(self)
        ...
        self.cycle_entretien = 0
        self.delai_entretien = 100
    ...
    def initialiser_partie(self, mondict):
        ...
        self.vaisseau_vue.creercouleurVaisseau()
    ...
    def boucler_sur_jeu(self):
        ...
        if self.onjoue:
            ...
            if self.cadrejeu > self.cycle_entretien:
                self.cycle_entretien = self.cadrejeu + self.delai_entretien
            elif self.cadrejeu == self.cycle_entretien:
                for joueur in self.modele.joueurs:
                    self.modele.joueurs[joueur].entretien_civ()
            self.afficher_annonce()
    ...
    def creer_satellite(self, type_satellite, id_etoile):
        self.actionsrequises.append([self.mon_nom, "creersatellite", [type_satellite,id_etoile]])

    def creer_batiment(self, type_batiment, id_etoile):
        self.actionsrequises.append([self.mon_nom, "creerbatiment", [type_batiment,id_etoile]])

    def creer_figure(self, type_figure):
        self.actionsrequises.append([self.mon_nom, "creerfigure", [type_figure]])

    def creer_merveille(self,type_merveille, id_etoile):
        self.actionsrequises.append([self.mon_nom, "creermerveille", [type_merveille,id_etoile]])
    ...
    def ajouter_offre_marche(self, echange,ressource,quantite):
        self.modele.marche.ajouter_echange(echange,ressource,quantite)

    def retirer_offre_marche(self, echange,ressource):
        self.modele.marche.retirer_echange(echange,ressource)

    def accepter_offre_marche(self,echange,acheteur_ajouter,acheteur_retirer):
        if echange.nom in self.joueurs:
            ressource_vendeur=self.vue.trouver_ressource(echange.ressource_demande)
            self.modele.marche.accepter_offre(echange,acheteur_ajouter,acheteur_retirer,ressource_vendeur)
        else:
            self.modele.marche.retirer_echange(echange,acheteur_ajouter)
            self.modele.marche.retirer_ressource(acheteur_retirer,echange.quantite_demande)

    def envoyer_message_chat(self,destination_msg,texte_message):
        self.modele.chat.envoyer_message(destination_msg,texte_message)

    def recevoir_message_chat(self,origine_msg,texte_message):
        self.modele.chat.recevoir_message(origine_msg,texte_message)

    def afficher_annonce(self):
        for key in self.modele.joueurs:
            joueur = self.modele.joueurs[key]
            if joueur.annonce is not None:
                joueur.annonce.tick()
                if joueur.annonce.supprimer:
                    joueur.annonce = None
