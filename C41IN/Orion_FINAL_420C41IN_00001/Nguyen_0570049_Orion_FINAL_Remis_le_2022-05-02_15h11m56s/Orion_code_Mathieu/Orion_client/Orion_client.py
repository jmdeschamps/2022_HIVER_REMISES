...
class Controleur():
    def __init__(self):
        ...
        self.partie_en_cours = 0   # est utilisée pour stopper la boucle de jeu
        self.maindelai = 50  # delai en ms de la boucle de jeu
        self.moduloappeler_serveur = 3  # frequence des appel au serveur, evite de passer son temps a communiquer avec le serveur
    ...
    def boucler_sur_lobby(self):
        ...
        if "courante" in mondict[0]:  # courante, la partie doit etre initialisée
    ...
    # methode speciale pour remettre les parametres du serveur à leurs valeurs par defaut
    ...
    def generer_nom(self):
        mon_nom = "JoueurId_" + str(random.randrange(100, 1000))
        return mon_nom
    ...
    # ajout
    def suivre_ennemmi(self, nom_joueur, id_vaisseau, id_mon_chasseur):
        self.actionsrequises.append([self.mon_nom, "suivre_ennemi", [nom_joueur, id_vaisseau, id_mon_chasseur]])

    def cibler_flotte(self, idorigine, iddestination, type_cible, nom_joueur, retour_prevu=None):
        self.actionsrequises.append([self.mon_nom, "ciblerflotte", [idorigine, iddestination, type_cible, nom_joueur, retour_prevu]])
    ...
    def lister_planetes_conquises(self,objet,id):
        self.vue.lister_planetes_conquises(objet,id)

    def choisir_ressource(self, type_ressource):
        self.actionsrequises.append([self.mon_nom, "choix_ressource", [type_ressource]])

    def gagner(self, nom_joueur):
        self.actionsrequises.append([self.mon_nom, "gagner", [nom_joueur]])
        self.vue.gagner_vue(nom_joueur)
        # action = [self.mon_nom, "abandonner", [self.mon_nom + ": J'ABANDONNE !"]]
        # self.actionsrequises = action

    def avertir_findepartie(self, objet):
        self.vue.avertir_findepartie(objet)
