    ...
    def afficher_jeu(self):
        ...
        for k in i.flotte:
            for j in i.flotte[k]:
                ...
                if k == "Explorateur":
                    ...
                    self.image_actuelle = self.images_joueur[self.mon_nom]["Vaisseaux"]["Explorateur"][3][angle_dir]
                    ...
                    angle_dir = int((math.degrees(j.angle_cible) / 9))
                    self.image_actuelle = self.images_joueur[self.mon_nom]["Vaisseaux"]["Combattant"][3][angle_dir]
                    self.canevas.create_image(j.x - size / 2, j.y - size / 2, anchor=NW, image=self.image_actuelle,
                                              tags=(j.proprietaire, str(j.id), "Flotte", k, "artefact"))
        ...
        for k in i.visible:
            ...
            if isinstance(k, Orion_modele.Explorateur):
                ...
                self.image_actuelle = self.images_joueur[k.proprietaire]["Vaisseaux"]["Explorateur"][3][angle_dir]
                ...
                angle_dir = int((math.degrees(j.angle_cible) / 9))
                self.image_actuelle = self.images_joueur[k.proprietaire]["Vaisseaux"]["Cargo"][3][angle_dir]
                self.canevas.create_image(j.x - size / 2, j.y - size / 2, anchor=NW, image=self.image_actuelle,
                                          tags=(j.proprietaire, str(j.id), "Flotte", k, "artefact"))

            elif isinstance(k, Orion_modele.Combattant):
                    angle_dir = int((math.degrees(k.angle_cible) / 9))
                    self.image_actuelle = self.images_joueur[k.proprietaire]["Vaisseaux"]["Combattant"][3][angle_dir]
                    self.canevas.create_image(k.x - size / 2, k.y - size / 2, anchor=NW, image=self.image_actuelle,
                                              tags=(k.proprietaire, str(k.id), "Flotte", k, "artefact"))
