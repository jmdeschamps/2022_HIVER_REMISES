...
class Controleur():
    ...
    def uptade_log_vue(self):
        self.vue.uptade_log()

    ############        VOTRE CODE      ######################

    def creer_bien(self, type_bien):
        self.actionsrequises.append([self.mon_nom, "creerbien", [type_bien]])
    ...
    def conquete(self,joueur,idorigine, idvaisseau, idcible,type_cible):
        self.actionsrequises.append([self.mon_nom,"conquete",[joueur,idorigine,idvaisseau,idcible,type_cible]])

    def afficher_systemes(self,joueur,cible):
        self.vue.afficher_etoile(joueur,cible)

    def lister_objet(self,objet,id):
        self.vue.lister_objet(objet,id)

    def creer_infrastructure(self, type_batiment, systeme):
        self.actionsrequises.append([self.mon_nom, "construirebatiment", [type_batiment, systeme.id]])

    def creer_recherche(self, type_recherche):
        self.actionsrequises.append([self.mon_nom, "fairerecherche", [type_recherche]])

    def update_v_info(self, id):
        joueur = self.modele.joueurs.get(self.mon_nom)
        vaisseau = 0

        for type_vaisseaux in joueur.flotte.keys():
            if id in joueur.flotte[type_vaisseaux]:
                vaisseau = joueur.flotte[type_vaisseaux][id]
        tab_info = [vaisseau.energie, vaisseau.espace_cargo, vaisseau.__class__.__name__]
        return tab_info

    def image_splash(self):
        return [int(random.random()*15), int(random.random()*15)]
